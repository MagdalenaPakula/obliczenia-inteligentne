...
class Magda2CIFAR(pl.LightningModule):
    def __init__(self, num_classes=10):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, padding=1),  # New layer
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.fc1 = nn.Linear(32 * 4 * 4, 2)  # Reduce features to 2
...
if __name__ == '__main__':
    ...
    if os.path.exists('best_model4.ckpt'):
        os.remove('best_model4.ckpt')
    model_checkpoint_callback = pl.callbacks.ModelCheckpoint(
        dirpath=dirpath,
        filename="best_model4",
        monitor='val_loss',
        save_top_k=1,
        mode='min',
    )

    trainer = pl.Trainer(max_epochs=11, enable_model_summary=True, callbacks=[tracker, model_checkpoint_callback])

    trainer.fit(model, data_module)

    plot_accuracy(tracker.acc)

    result = trainer.test(model, data_module, verbose=False, ckpt_path='best_model4.ckpt')
