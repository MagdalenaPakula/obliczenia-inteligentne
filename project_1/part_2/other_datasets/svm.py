...
def experiment_2(dataset, dataset_name):
    ...
    plt.ylabel('score')
    plt.legend(loc='best')
    plt.title(f'SVM on dataset {dataset_name}')
    plt.show()
    plt.close()
    ...
    for set_choice, (features, labels, scores) in data.items():
        ...
        for classifier_choice, classifier in selected_classifiers.items():
            plot_decision_boundary(lambda x: classifier.predict(x), features, labels,
                                   title=f'{classifier_choice} SVM on {dataset_name} - {set_choice} set')
            plt.show()
            plt.close()
            ...
            plt.title(f"Confusion Matrix for {classifier_choice} SVM on {dataset_name} - {set_choice} set")
