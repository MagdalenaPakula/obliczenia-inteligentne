...
\begin{figure}[ht]\centering
    \includegraphics[width=.5\linewidth]{img/saliency_flat/0.png}
    \includegraphics[width=.5\linewidth]{img/saliency_flat/4.png}
    \includegraphics[width=.5\linewidth]{img/saliency_flat/9.png}
...
\begin{figure}[ht]\centering
    \includegraphics[width=.6\linewidth]{img/saliency_lbp/new/img}
    \includegraphics[width=.6\linewidth]{img/saliency_lbp/new/img_1}
    \includegraphics[width=.6\linewidth]{img/saliency_lbp/new/img_8}
    \includegraphics[width=.6\linewidth]{img/saliency_lbp/new/img_6}
    \includegraphics[width=.6\linewidth]{img/saliency_lbp/new/img_2}
    \caption{LBP images and gradient attributions for digits 0, 6, 8, 4 and 9.}\label{fig:lbp-attributions}
\end{figure}
...
The results for digits 0, 6, 8, 4 and 9 are depicted on fig.~\ref{fig:lbp-attributions}.
As LBP method creates the binary pattern on images, we can see improvement or regression when it comes to visibility of the image's digit.
When comparing these saliency maps to the digits presented on fig~\ref{fig:flatten-attributions} we can certainly notice the improvement of models' understaing of each digit.
Digit 0 is recognized by the same pattern as in flatten extraction, digit 9 has more areas that are combined rather than separate pixels like in previous example.
It is worth noticing that image of digit 4 and 9 are quite similiar, but due to LBP feature extraction, the model enhanced the pixels intensity and its differences such as top part of both images, where the digit 4 does not connect, while digit 9 does.
We can notice on these images that saliency map also takes into the account the background of the image, meaning whether the line or pattern does not exist on the image, therefore the model takes prediction.
This happens particularly on the digit 6, where model checks for a pattern at the top of this digit to see whether the top line is connected or not.

Not only LBP may enhance the interpretability and effectiveness of texture analysis tasks, but seeing subtle changes in model's view of the digit when applying LBP may be crucial in further development of this method.
Moreover, such not aggressive change like LBP feature extraction is seen to be more precise, taking into account areas, not pixels and not as random as on fig~\ref{fig:flatten-attributions}, which proves the usefulness of this method.

Those results show that the model is not just focusing on the most obvious features, but also on the subtle patterns that are not immediately apparent by a human eye.
...
\begin{figure}[ht]\centering
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/4.png}
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/5.png}
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/1.png}
...
\begin{figure}[ht]\centering
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/4_1.png}
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/4_2.png}
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/4_3.png}
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/4_4.png}
    \includegraphics[width=.5\linewidth]{img/saliency_mnist/4_5.png}
...
In order to formulate more general conclusions about the model, it was tested on multiple different versions of the same digit.
Figure~\ref{fig:integrated-grad-mnist-multiple} shows integrated gradients for different versions of digit 3.
It can be seen that in all of the cases model pays significant importance to the top part of the digit, in the majority of cases it pays attention to the middle of the digit, where 2 curves are, which alignes with the conclusion for digit 3 in the above fig.~\ref{fig:mnist-cnn-integrated-grad}.
In all cases the negative areas suggest that sharp line, dynamic change rather than subtle curves adversely impacts the CNN model.

\begin{figure}[ht]\centering
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/3/img}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/3/img_1}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/3/img_2}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/3/img_3}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/3/img_4}
    \caption{Different cases of digit 3}\label{fig:integrated-grad-mnist-multiple}
\end{figure}

Moreover, we also tested the model on digit 5, which is presented on the fig.~\ref{fig:integrated-grad-mnist-multiple-5}.
In this case, 5 might be similar in some ways to digit 3.
However we can clearly see that when it comes to digit 5 the model pays the highest importance to the straight top line.
Also important are: the slightly curved line at the bottom but also to the middle curve.
This digit is particularly interesting due to the huge differences in writing's style which might be a challenge for a model.
Nonetheless, the model handles those digits exceptionally well and pays attention to the same areas.

\begin{figure}[ht]\centering
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/5/img}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/5/img_1}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/5/img_2}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/5/img_3}
    \includegraphics[width=.6\linewidth]{img/integrated_grad/mnist_CNN/5/img_4}
    \caption{Different cases of digit 5}\label{fig:integrated-grad-mnist-multiple-5}
\end{figure}
